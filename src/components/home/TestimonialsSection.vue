    ...
    <div class="w-full h-auto rounded mt-18 bg-[#EADBBD] px-12 py-8">
      ...
      <div class="relative mt-8 hidden md:block">
        <div @click="prevTestimonial" class="absolute left-0 top-1/2 transform -translate-y-1/2 bg-gra bg-opacity-50 text-white p-2 rounded-full z-10 cursor-pointer">
          <img src="@/assets/chevron-left.png" alt="Left Arrow" class="h-8">
        </div>
        <div @click="nextTestimonial" class="absolute right-0 top-1/2 transform -translate-y-1/2 bg-gra bg-opacity-50 text-white p-2 rounded-full z-10 cursor-pointer">
          <img src="@/assets/chevron-right.png" alt="Right Arrow" class="h-8">
        </div>
      ...
      <div class="relative mt-8 block md:hidden">
        <div @click="prevTestimonialMobile" class="absolute left-0 top-1/2 transform -translate-y-1/2 bg-gra bg-opacity-50 text-white p-2 rounded-full z-10 cursor-pointer">
          <img src="@/assets/chevron-left.png" alt="Left Arrow" class="h-8">
        </div>
        <div @click="nextTestimonialMobile" class="absolute right-0 top-1/2 transform -translate-y-1/2 bg-gra bg-opacity-50 text-white p-2 rounded-full z-10 cursor-pointer">
          <img src="@/assets/chevron-right.png" alt="Right Arrow" class="h-8">
        </div>
        ...
        <div class="overflow-hidden">
          <div class="flex transition-transform duration-500" :style="{ width: `${mobileTestimonials.length * 100}%`, transform: `translateX(-${currentMobileTestimonial * (100 / mobileTestimonials.length)}%)` }">
            <div v-for="(testimonial, index) in mobileTestimonials" :key="index" class="w-full flex justify-center items-center">
              <div class="w-[295px] h-auto rounded bg-0 space-y-2 flex flex-col items-center">
                <img :src="testimonial.image1" alt="Avatar" class="w-[91px] h-[87px]">
                <h1 class="font-extrabold text-[18px] text-center text-foundationGreyDarker">{{ testimonial.name }}</h1>
                <img :src="testimonial.image2" alt="Image" class="w-[136px]">
        ...
        <div class="flex justify-center mt-4">
          <ul class="flex space-x-2">
            <li v-for="(testimonial, index) in mobileTestimonials" :key="index" @click="goToTestimonialMobile(index)" :class="{'bg-gray-800': index === currentMobileTestimonial, 'bg-foundationGreyLightActive': index !== currentMobileTestimonial}" class="w-3 h-3 rounded-full cursor-pointer"></li>
          </ul>
        </div>
      </div>
    </div>
    ...
    data() {
      return {
        ...
        testimonialGroups: [
          ...
          // Add more testimonial groups if needed
        ],
        mobileTestimonials: [
          {
            image1: require('@/assets/Ellipse-2.png'),
          ...
          // Add more mobile testimonials if needed
        ],
      };
    },
    methods: {
      prevTestimonial() {
        this.currentTestimonial = (this.currentTestimonial === 0) ? this.testimonialGroups.length - 1 : this.currentTestimonial - 1;
      },
      nextTestimonial() {
        this.currentTestimonial = (this.currentTestimonial === this.testimonialGroups.length - 1) ? 0 : this.currentTestimonial + 1;
      },
      goToTestimonial(index) {
        this.currentTestimonial = index;
      },
      prevTestimonialMobile() {
        this.currentMobileTestimonial = (this.currentMobileTestimonial === 0) ? this.mobileTestimonials.length - 1 : this.currentMobileTestimonial - 1;
      },
      nextTestimonialMobile() {
        this.currentMobileTestimonial = (this.currentMobileTestimonial === this.mobileTestimonials.length - 1) ? 0 : this.currentMobileTestimonial + 1;
      },
