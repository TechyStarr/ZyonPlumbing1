<template>
    <div class="w md:w-full rounded mt-18 bg-gray-100 pt-12 p-6  md:p-12">
      <div class="flex space-x-5 justify-center items-center">
        <hr class="w-[50px] border-t-[3px] border-gray-400 mr-4">
        <h1 class="font-extrabold text-[18px] md:text-[32px]">READ LATEST NEWS</h1>
        <hr class="w-[50px] border-t-[3px] border-gray-400 mr-4">
      </div>
  
      <div class="flex font-nunito mt-12 space-x-3 ml-8">
        <p class="w-[61px] h-[30px] bg-customGold text-[16px] hover:bg-customGold text-white rounded-full flex items-center justify-center">
          All
        </p>
        <p class="w-[61px] h-[30px] bg-[#181818] hover:bg-customGold text-white rounded-full flex items-center justify-center">
          Tips
        </p>
        <p class="w-[122px] h-[30px] bg-[#181818] hover:bg-customGold text-white rounded-full flex items-center justify-center">
          Maintenance
        </p>
        <p class="w-[62px] h-[30px] bg-[#181818] hover:bg-customGold text-white rounded-full flex items-center justify-center">
          News
        </p>
      </div>
  
      <div class="flex justify-center px-8">
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          <div v-for="article in articles" :key="article.id" class="mt-12 space-y-2 text-left">
            <div class="relative w-[387px] h-[289px]">
              <img :src="require(`@/assets/${article.image}`)" alt="Article image" class="w-full h-full rounded">
              <p class="absolute top-2 left-2 bg-foundationGoldNormal text-[16px] md:text-[18px] text-white font-bold py-2 px-4 rounded-full">
                {{ article.category }}
              </p>
            </div>
            <h1 class="font-semibold text-[16px] md:text-[18px] text-foundationGreyNormal">{{ article.date }}</h1>
            <router-link class="font-semibold text-[16px] md:text-[18px] hover:text-customGold" :to="`/blog/${article.id}`">
              {{ article.title }}
            </router-link>
            <div class="flex space-x-3 items-center">
              <p class="text-[16px] md:text-[18px] text-foundationGoldDarker font-nunito">Read More</p>
              <img src="@/assets/chevron-right.png" alt="Check square" class="h-5 md:h-6">
            </div>
          </div>
        </div>
      </div>
  
      <div class="flex mt-20 justify-center">
        <img src="@/assets/chevron-left.png" alt="Check square" class="h-8">
        <img src="@/assets/chevron-right.png" alt="Check square" class="h-8">
      </div>
    </div>
  </template>
  
  <script>
  
  export default {
    name: 'BlogSection',
    components: {
    },
    data() {
      return {
        articles: [
          {
            id: '1',
            title: 'The Benefits of Regular Plumbing Maintenance',
            date: 'JULY 13, 2024',
            image: 'image-12.png',
            category: 'Maintenance'
          },
          {
            id: '2',
            title: 'Signs Your Home Needs Repiping: What to Look For',
            date: 'JULY 13, 2024',
            image: 'image-12.png',
            category: 'Maintenance'
          },
          {
            id: '2',
            title: 'Signs Your Home Needs Repiping: What to Look For',
            date: 'JULY 13, 2024',
            image: 'image-12.png',
            category: 'Maintenance'
          },
          {
            id: '2',
            title: 'Signs Your Home Needs Repiping: What to Look For',
            date: 'JULY 13, 2024',
            image: 'image-12.png',
            category: 'Maintenance'
          },
          // Add more articles here
        ]
      }
    }
  }
  </script>
  
  <style scoped>
  /* Add any necessary styles here */
  </style>
  